define([ "require", "utils/Utils", "utils/Globals", "d3", "marionette", "jquery-ui", "jstree" ], function(require, Utils, Globals, d3) {
    "use strict";
    _.mixin({
    ...
    if ($("body").on("click DOMMouseScroll mousewheel", function(e) {
    ...
    }), $.fn.select2.amd.define("TagHideDeleteButtonAdapter", [ "select2/utils", "select2/selection/multiple", "select2/selection/placeholder", "select2/selection/eventRelay", "select2/selection/search" ], function(Utils, MultipleSelection, Placeholder, EventRelay, SelectionSearch) {
        ...
        this.redraw_node = function(obj, deep, callback, force_draw) {
            var node_id = obj, el = parent.redraw_node.apply(this, arguments);
            if (el) {
                var node = this._model.data[node_id], cfg = this.settings.node_customize, key = cfg.key, type = node && node.original && node.original[key], customizer = type && cfg.switch[type] || cfg.default;
                customizer && customizer(el, node);
            }
            return el;
        };
