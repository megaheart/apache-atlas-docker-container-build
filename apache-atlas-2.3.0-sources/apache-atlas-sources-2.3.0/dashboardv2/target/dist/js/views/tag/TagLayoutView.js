define([ "require", "backbone", "hbs!tmpl/tag/TagLayoutView_tmpl", "utils/Utils", "utils/Messages", "utils/Globals", "utils/UrlLinks", "collection/VTagList" ], function(require, Backbone, TagLayoutViewTmpl, Utils, Messages, Globals, UrlLinks, VTagList) {
    "use strict";
    var TagLayoutView = Backbone.Marionette.LayoutView.extend({
        ...
        bindEvents: function() {
            ...
            this.listenTo(this.collection.fullCollection, "reset add remove", function() {
                this.tagsGenerator(), this.changeLoaderTextState(!1);
                ...
                that.changeLoaderTextState(!1);
            });
        },
        onRender: function() {
            this.changeLoaderTextState(!1), this.changeLoaderState(!0), this.bindEvents(), this.checkTagOnRefresh();
        },
        checkTagOnRefresh: function() {
            var that = this, tagName = this.options.tag, presentTag = this.collection.fullCollection.findWhere({
                name: tagName
            }), tag = new VTagList();
            !presentTag && tagName ? (tag.url = UrlLinks.classicationApiUrl(tagName), tag.fetch({
                success: function(dataOrCollection, tagDetails) {
                    that.collection.fullCollection.add(tagDetails), that.changeLoaderTextState(!0);
                },
        ...
        changeLoaderState: function(showLoader) {
            showLoader ? (this.ui.tagTreeLoader.show(), this.ui.tagTreeView.hide()) : (this.ui.tagTreeLoader.hide(), 
            this.ui.tagTreeView.show());
        },
        changeLoaderTextState: function(showLoader) {
            showLoader ? this.ui.tagTreeTextLoader.show() : this.ui.tagTreeTextLoader.hide();
        },
        fetchCollections: function() {
            this.changeLoaderState(!0), this.ui.refreshTag.attr("disabled", !0), this.collection.fetch({
                reset: !0
        ...
        manualRender: function(options) {
            ...
            options && options.viewType && (this.viewType = options.viewType, this.changeLoaderTextState(!1)), 
            this.createTag || this.setValues(!0);
        },
        ...
        setValues: function(manual) {
            ...
            if (Utils.getUrlState.isTagTab()) if (this.tag) {
                ...
                el.find("li").removeClass("active"), el.find("li.parent-node").each(function() {
                    ...
                    target.parents("ul").addClass("show").removeClass("hide"), !1;
                });
        ...
        tagsGenerator: function(searchString) {
            ...
            this.changeLoaderState(!1), this.ui.refreshTag.attr("disabled", !1);
        },
        ...
        generateTree: function(options) {
            var data = options.data, isTree = options.isTree, searchString = options.searchString, that = this, element = "", getElString = function(options) {
                ...
                return '<li class="parent-node" data-id="tags"><div><div class="tools"><i class="fa fa-ellipsis-h tagPopover"></i></div>' + (hasChild ? '<i class="fa toggleArrow fa-angle-right" data-id="expandArrow" data-name="' + name + '"></i>' : "") + '<a href="#!/tag/tagAttribute/' + name + "?viewType=" + (isTree ? "tree" : "flat") + "&searchType=basic&tag=" + name + '" data-name="' + name + '">' + name + "</a></div>" + (isTree && hasChild ? '<ul class="child hide">' + that.generateTree({
        ...
        onClickCreateTag: function(e) {
            ...
            $(e.currentTarget).attr("disabled", "true"), require([ "views/tag/CreateTagLayoutView", "modules/Modal" ], function(CreateTagLayoutView, Modal) {
                ...
                modal.$el.find("button.ok").attr("disabled", "true"), view.ui.tagName.on("keyup input", function(e) {
                    ...
                    modal.$el.find("button.ok").showButtonLoader(), that.onCreateButton(view, modal);
        ...
        onCreateButton: function(ref, modal) {
            ...
            this.name = ref.ui.tagName.val(), this.description = Utils.sanitizeHtmlContent({
        ...
        onNotifyOk: function(data) {
            var that = this, deleteTagData = this.collection.fullCollection.findWhere({
                name: this.tag
            }), superTypeOfDeleteTag = deleteTagData.get("superTypes"), superTypeObj = superTypeOfDeleteTag ? this.collection.fullCollection.findWhere({
            ...
            deleteTagData.deleteTag({
                ...
                cust_error: function() {},
                complete: function() {
                    0 === that.collection.fullCollection.length && that.setUrl("#!/tag", !0), that.notificationModal.hideButtonLoader(), 
